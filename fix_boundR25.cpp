...
struct ObserverFunctor
{
    ...
    void operator() (state_type_2d_ofp &state, double t)
    {
        ...
        double x_def=boxsizehalf;
        double y_def=boxsizehalf;
        ...
        //get point for defect as lowest value for polarity in bulk
            ...
            for(int n=0; n<bulk.size(); n++){
                ...
                if(mag_pol<lowest_pol){
                    lowest_pol = mag_pol;
                    x_def=Particles.getPos(pb)[0];//+spacing/2;
                    y_def=Particles.getPos(pb)[1];//-spacing/2;
                    ctr_pol++;
                }
            }
            v_cl.sum(x_def);
            v_cl.sum(y_def);
            v_cl.execute();
            if (ctr_pol==0){ctr_pol=1;}
            double cl_norm = v_cl.size();
            x_def=x_def/(cl_norm*ctr_pol);
            y_def=y_def/(cl_norm*ctr_pol);
        ...
        if (t != 0) {
            ...
            for (int s=0; s<bulk.size(); s++){
              ...
              //compute normal
              double x1 = (x_def - Xpn[0])/dist_def;
              double x2 = (y_def - Xpn[1])/dist_def;
...
int main(int argc, char* argv[])
{
    {   openfpm_init(&argc,&argv); //initialize openfpm
        ...
        for (int j = 0; j < Bulk.size(); j++) {
            ...
            }else{Particles.getProp<DELMU>(p)=-0.05;//-tanh(0.5*(dist2))/(tanh(0.5*(dist3)));}//((0.05/4)*pow((dist-2.), 2)-0.05);}
