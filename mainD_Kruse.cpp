...
constexpr int x = 0;
constexpr int y = 1;
constexpr int POLARIZATION= 0,VELOCITY = 1, VORTICITY = 2, NORMAL = 3,PRESSURE = 4, STRAIN_RATE = 5, STRESS = 6, MOLFIELD = 7, DPOL = 8, DV = 9, VRHS = 10, F1 = 11, F2 = 12, F3 = 13, F4 = 14, F5 = 15, F3_OLD = 16, V_T = 17, DIV = 18, DELMU = 19, HPB = 20, FE = 21, R = 22, PID = 23, POLD = 24, FESSPLAY = 25, FESBEND = 26;
...
PropNAMES={"00-Polarization","01-Velocity","02-Vorticity","03-Normal","04-Pressure","05-StrainRate","06-Stress","07-MolecularField","08-DPOL","09-DV","10-VRHS","11-f1","12-f2","13-f3","14-f4","15-f5","16-f3_old","17-V_T","18-DIV","19-DELMU","20-HPB","21-FrankEnergy","22-R","23-particleID","24-P_old", "25-FesSplay", "26-FesBend"};
typedef aggregate<VectorS<2, double>,VectorS<2, double>,double[2][2],VectorS<2, double>,double,double[2][2],double[2][2],VectorS<2, double>,VectorS<2, double>,VectorS<2, double>,VectorS<2, double>,double,double,double,double,double,double,VectorS<2, double>,double,double,double,double,double,int,VectorS<2, double>,double, double> Activegels;
...
struct ObserverFunctor
{
    ...
    void operator() (state_type_2d_ofp &state, double t)
    {
        ...
        Point<2,double> Xpn,X2pn,xpd1={5.,5.};//define points and middle points for weighting 
        spacing = 10./Gd;//calculate spacing
        if (t != 0) {
            ...
            for (int s=0; s<bulk.size(); s++){
              ...
              if(sqrt((Particles.getPos(p)[0]-5.0)*(Particles.getPos(p)[0]-5.0)+(Particles.getPos(p)[1]-5.0)*(Particles.getPos(p)[1]-5.0))>3.5){
...
int main(int argc, char* argv[])
{
    {   openfpm_init(&argc,&argv); //initialize openfpm
        ...
        V_err_eps=std::atof(argv[8]); //change dependent of Gd
        timetol=std::atof(argv[9]);
        double timereltol=std::atof(argv[10]);
        ...
        double spacing = box.getHigh(0) / (sz[0]),rCut = 3.9 * spacing; //get spacing
        ...
        Point<2,double> Xpn,xpd1={5.,5.}; //middle point

        //random number generator
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_real_distribution<> gridrandomnes(-0.1, 0.1);
