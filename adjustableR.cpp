//
// Created by Abhinav Singh on 15.03.20., modified by Birte Geerds in 2023
...
struct PolarEv
{
    ...
    void operator()( const state_type_2d_ofp &X , state_type_2d_ofp &dxdt , const double t ) const
    {
        ...
        for (int j = 0; j < CorrVec.size(); j++) {
                ...
                // Particles.getProp<8>(p_out)[x]=Particles.getProp<8>(p)[x];
                // Particles.getProp<8>(p_out)[y]=Particles.getProp<8>(p)[y];
        ...
        //impose Neumann BC with CorrVec
       for (int i = 0; i < CorrVec.size(); ++i)
               {
                   auto p_boundary = CorrVec.get<0>(i)[0];
                   auto p_bulk = CorrVec.get<0>(i)[1];

                   // Particles.getProp<DPOL>(p_bulk)[x] = Particles.getProp<DPOL>(p_boundary)[x];
                   // Particles.getProp<DPOL>(p_bulk)[y] = Particles.getProp<DPOL>(p_boundary)[y];
...
struct ObserverFunctor
{
    ...
    //Constructor
    ObserverFunctor(DX &Dx,DY &Dy,DXX &Dxx,DXY &Dxy,DYY &Dyy):Dx(Dx),Dy(Dy),Dxx(Dxx),Dxy(Dxy),Dyy(Dyy)
    {
        t_old = 0.0;
        ctr = 0;
    }
    ...
    void operator() (state_type_2d_ofp &state, double t)
    {
        ...
        //spacing = boxsize/Gd;//calculate spacing
        double x_def;
        double y_def;
        ...
        Point<2, double> xpdef = {x_def, y_def};

        //get point for defect as lowest value for polarity in bulk
            double lowest_pol_local = 0.3; // Initialize the local minimum with a high value
			double lowest_pol_global = 0.3; // Initialize the global minimum with the same high value
            double ctr_pol=0;
        ...
        // Gather local minimum values and their coordinates to find the global minimum
		MPI_Allreduce(&lowest_pol_local, &lowest_pol_global, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
		MPI_Allreduce(&x_def_local, &x_def_global, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
		MPI_Allreduce(&y_def_local, &y_def_global, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);

		if (lowest_pol_local == lowest_pol_global) {
		    // The current processor contains the global minimum
		    x_def = x_def_global;
		    y_def = y_def_global;
		}

		// Print the global minimum value and coordinates (only on the processor with the global minimum)
		if (lowest_pol_local == lowest_pol_global) {
		    cout << "Global minimum polarization value: " << lowest_pol_global << '\n';
		    cout << "Global minimum coordinates: (" << x_def << ", " << y_def << ")\n";
		}
        ...
        if (t != 0) {
            ...
            for (int s=0; s<bulk.size(); s++){
              ...
              dist_def = (dist_def == 0) ? 1 : dist_def;
                ...
                // if(sqrt((Particles.getPos(p)[0]-5.0)*(Particles.getPos(p)[0]-5.0)+(Particles.getPos(p)[1]-5.0)*(Particles.getPos(p)[1]-5.0))>4.0){
                //     double mag=sqrt(Particles.getProp<POLARIZATION>(s)[x]*Particles.getProp<POLARIZATION>(s)[x]+Particles.getProp<POLARIZATION>(s)[y]*Particles.getProp<POLARIZATION>(s)[y]);
                //     Particles.getProp<POLARIZATION>(s)[x]=Particles.getProp<POLARIZATION>(s)[x]/mag;
                //     Particles.getProp<POLARIZATION>(s)[y]=Particles.getProp<POLARIZATION>(s)[y]/mag;
                //}
            ...
            for (int j = 0; j < CorrVec.size(); j++) {
                ...
                // // if (Particles.getProp<F3>(p) > M_PI/2 && Particles.getProp<0>(p)[x]<0){
                //     // double phi_d = Particles.getProp<F3>(p)- M_PI/2;
                //     // double phi_new=M_PI/2 - phi_d;
                //     // Particles.getProp<F3>(p)=phi_new;
                //     // Particles.getProp<0>(p)[x]=cos(phi_new)*x1 - sin(phi_new)*x2;
                //     // Particles.getProp<0>(p)[y]=sin(phi_new)*x1 + cos(phi_new)*x2;
                // }else if (Particles.getProp<F3>(p) > M_PI/2){// && Particles.getProp<0>(p)[x]>0){
                //     //double phi_d = Particles.getProp<F3>(p)- M_PI/2;
                //     double phi_new=M_PI - Particles.getProp<F3>(p);
                //     Particles.getProp<F3>(p)=phi_new;
                //     // Particles.getProp<0>(p)[x]=x1 + pmag * cos(angle);
                //     // Particles.getProp<0>(p)[y]=x2 + pmag * sin(angle);
                //     Particles.getProp<0>(p)[x]=cos(phi_new)*x1 - sin(phi_new)*x2;
                //     Particles.getProp<0>(p)[y]=sin(phi_new)*x1 + cos(phi_new)*x2;
                // }
                ...
                // Particles.getProp<8>(p_out)[x]=Particles.getProp<8>(p)[x];
                // Particles.getProp<8>(p_out)[y]=Particles.getProp<8>(p)[y];
                // Particles.getProp<F3>(p_out)=angle;

                Particles.getProp<0>(p)[x]=Particles.getProp<0>(p_out)[x];
                Particles.getProp<0>(p)[y]=Particles.getProp<0>(p_out)[y];
                Particles.getProp<F3>(p)=Particles.getProp<F3>(p_out);

                // update boundary as ghost particles, same as outer particles of bulk
        ...
        Particles.ghost_get<POLARIZATION, F3>(SKIP_LABELLING);
        std::cout << "point of the defect is: x="<< x_def << " , y=" <<y_def<< "\n";
        ...
        for (int k=0; k<boundary.size();k++){
            // auto b= boundary.get<0>(k);
            // double magn=sqrt(Particles.getProp<POLARIZATION>(k)[x]*Particles.getProp<POLARIZATION>(k)[x]+Particles.getProp<POLARIZATION>(k)[y]*Particles.getProp<POLARIZATION>(k)[y]);
            // Particles.getProp<POLARIZATION>(k)[x]=Particles.getProp<POLARIZATION>(k)[x]/magn;
            // Particles.getProp<POLARIZATION>(k)[y]=Particles.getProp<POLARIZATION>(k)[y]/magn;
        ...
        if (minimum){std::cout << "minimum true" << '\n';} //print out whether minimum is reached
        if (ctr>800){std::cout << "ctr true" << '\n';} //print, whether respective countr is reached
        double anglesum_diff = fabs(anglesum_old - anglesum); //calculate difference in anglesum
        std::cout << "anglesum_diff = " << anglesum_diff << '\n'; //...and print this out

        // //define steady state: if sum of anglesumdiff is smaller then threshhold 
        // // -> steady state and end simulation
        // if (ctr >500 &&anglesum_diff < 1e-6){
        //     tt2.stop();
        //     if(v_cl.rank()==0)
...
int main(int argc, char* argv[])
{
    {   openfpm_init(&argc,&argv); //initialize openfpm
        ...
        double spacing = box.getHigh(0) / (sz[0]),rCut = 3.6 * spacing; //get spacing
        ...
        for (int j = 0; j < Bulk.size(); j++) {
            ...
            if(dist>(boxsize-10.)||dist<10.){
                Particles.getProp<DELMU>(p)=-0.0;
            }else{Particles.getProp<DELMU>(p)=-0.0;//-tanh(0.5*(dist2))/(tanh(0.5*(dist3)));}//((0.05/4)*pow((dist-2.), 2)-0.05);}
            ...
            if(dist2>1){
                ...
                double x1 = (boxsizehalf - Xpn[0])/dist * snormal;
                double x2 = (boxsizehalf - Xpn[1])/dist * snormal;
            ...
            if (dist2<=5){
                // if (dist2+dist<5){
                //     double snormal_defect = tanh(0.5*(dist2))/(tanh(0.5*(5+dist2-dist)));
                // }else{
                //     double snormal_defect = tanh(0.5*(dist2))/(tanh(0.5*(5-dist2+dist)));
                // }
                ...
                // Particles.getProp<POLARIZATION>(p)[x] = r*cos(phi);
                // Particles.getProp<POLARIZATION>(p)[y] = r*sin(phi);
                Particles.getProp<POLARIZATION>(p)[x] = cos(phi)*xd - sin(phi)*yd;
                Particles.getProp<POLARIZATION>(p)[y] = sin(phi)*xd + cos(phi)*yd;
                Particles.getProp<F3>(p) = phi;
                // double smooth = 1/(exp((dist2 - 2.)*2) +1);
                // double v = (dis(gen)) * M_PI * smooth;//use for positive delta K
                // //double v = (dis(gen)) * M_PI * smooth;//use for negative delta K
                // Particles_bulk.getProp<POLARIZATION>(p)[x] = cos(v)*Particles_bulk.getProp<POLARIZATION>(p)[x] - sin(v)*Particles_bulk.getProp<POLARIZATION>(p)[y];
                // Particles_bulk.getProp<POLARIZATION>(p)[y] = sin(v)*Particles_bulk.getProp<POLARIZATION>(p)[x] + cos(v)*Particles_bulk.getProp<POLARIZATION>(p)[y];
                // Particles.getProp<F3>(p) = phi+v;
            ...
            //connect particles for Neumann BC
            if (Particles.getProp<PID>(p)!=0){
                double xp = Xpn[0];
                double yp = Xpn[1];
        ...
        ObserverFunctor<Derivative_x,Derivative_y,Derivative_xx,Derivative_xy,Derivative_yy> Observer(Dx,Dy,Dxx,Dxy,Dyy);
