...
struct ObserverFunctor
{
    ...
    void operator() (state_type_2d_ofp &state, double t)
    {
        ...
        // //define steady state: if sum of anglesumdiff is smaller then threshhold, enable if steady state should be found automatically
...
int main(int argc, char* argv[])
{
    {   openfpm_init(&argc,&argv); //initialize openfpm
        ...
        for (int j = 0; j < Bulk.size(); j++) {
            ...
            if(dist>(boxsizehalf)||dist<8.*spacing){
                Particles.getProp<DELMU>(p)=delmu;
        ...
        // }
        //call odeint library for solving the equations, choose another solver, if necessary
